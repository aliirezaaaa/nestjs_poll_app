...
import { GatewayAdminGuard } from 'src/polls/gateWay-admin.guard';
import { NominationDto } from 'src/polls/dtos';
...
export class PollsGateway
  ...
  async handleConnection(client: SocketWithAuth) {
    ...
    this.io.to(roomName).emit('Updated_Poll', updatedPoll);
  }
  ...
  async handleDisconnect(client: SocketWithAuth) {
    ...
    if (updatedPoll) {
      this.io.to(pollID).emit('Updated_Poll', updatedPoll);
    }
  }
  ...
  async removeParticipant(
    ...
    if (updatedPoll) {
      this.io.to(client.pollID).emit('Updated_Poll', updatedPoll);
    }
  }
  ...
  async cancelPoll(@ConnectedSocket() client: SocketWithAuth): Promise<void> {
    ...
    this.io.to(client.pollID).emit('poll canceled');
  }
}
